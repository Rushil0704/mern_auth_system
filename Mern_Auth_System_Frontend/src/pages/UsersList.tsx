...
export const UserList = () => {
        ...
        const fetchUsers = async () => {
            ...
            try {
                const res = await axios.get(
                    `${import.meta.env.VITE_BASE_ADMIN_URL}users`,
                    {
    ...
    const deleteFn = async (userId: string) => {
        const result = await Swal.fire({
            title: `<span style="color:${
                isDark ? "#fff" : "#000"
            };font-weight:bold;font-size:1.3rem">Are you absolutely sure?</span>`,
            html: `<span style="color:${
                isDark ? "#b0b3c6" : "#4a4a4a"
            }">This action will delete your account. You cannot undo this!</span>`,
            icon: "warning",
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            default:
                return "bg-bgLight text-gray-600";
        }
    };
    ...
    return (
        <div className="sm:p-10 p-5 gap-4 sm:gap-5 min-h-[calc(100dvh-75px)] flex flex-col text-gray-900 dark:text-white bg-[radial-gradient(ellipse_at_50%_50%,_#f3f4f6_0%,_#f9fafb_100%)] dark:bg-[radial-gradient(ellipse_at_70%_30%,_#23283b_0%,_#10131a_100%)]">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-6">
                {/* Title */}
                <h2 className="sm:text-3xl text-xl font-bold dark:bg-gradient-to-r dark:from-textGrFrom dark:to-textGrTo bg-[linear-gradient(92.77deg,#080C1D_7.73%,#7E808C_99.72%)] bg-clip-text text-transparent select-none text-center sm:text-start">
                    User List
                </h2>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 w-full md:w-auto">
                    ...
                    <div className="relative w-full sm:w-72">
                        ...
                        />
                        <Search
                            className="absolute left-4 top-1/2 -translate-y-1/2 text-textGrayMedium dark:text-blue-400 z-10"
                            size={22}
                        />
                        {searchTerm && (
                            <X
            ...
            <div className="flex flex-wrap gap-2 sm:gap-4 items-center justify-between">
                <div className="flex items-center gap-1 sm:gap-3">
                    <label
                        htmlFor="per-page"
                        className="font-medium text-black-600 dark:text-blue-200 whitespace-nowrap sm:text-base text-sm"
                    >
                        Per page:
                    </label>
                ...
                <span className="text-black-600 dark:text-blue-200 ml-auto sm:text-base text-sm">
                    ...
                    {Math.min(currentPage * dataPerPage, totalUsers || 0)} of{" "}
                    {totalUsers || 0} users
                </span>
            </div>
            ...
            <div className="rounded-lg bg-bgLight dark:bg-darkBorder shadow-sm border border-btnGrFromLight dark:border-darkBorder -mt-2 overflow-x-auto">
                <table className="min-w-[800px] w-full border-collapse text-sm md:text-base">
                        ...
                        <tr className="flex w-full justify-between">
                            ...
                            >
                                <div className="flex items-center gap-1">
                                    ...
                                    {sortField === "firstName" ? (
                                        sortOrder === "asc" ? (
                                            <ArrowUp className="w-4 h-4 dark:text-blue-400" />
                                        ) : (
                                            <ArrowDown className="w-4 h-4 dark:text-blue-400" />
                                        )
                                    ) : (
                    ...
                    <tbody className="block w-full h-[244px] overflow-y-scroll scrollbar-thin divide-y divide-btnGrFromLight dark:divide-darkBorder bg-bgLight dark:bg-darkBorder hide-scrollbar">
                        {loading ? (
                            Array.from({ length: dataPerPage }).map(
                            ...
                            users.map((user, index) => (
                                ...
                                >
                                    <td className="flex-[0_0_140px] md:flex-[0_0_150px] px-3 md:px-6 py-2 md:py-2">
                                        {user.imgUrl ? (
                                            <img
                                                src={`https://s3.tebi.io/mydemo/users/${user.imgUrl}`}
                                                alt="User"
                                    ...
                                    <td className="flex-[0_0_140px] md:flex-[0_0_150px] px-3 md:px-6 py-2 md:py-2 truncate">
                                        {user.firstName}
                                    </td>
                                    <td className="flex-[0_0_140px] md:flex-[0_0_150px] px-3 md:px-6 py-2 md:py-2 truncate">
                                        {user.lastName}
                                    </td>
                                    <td className="flex-[0_0_200px] md:flex-[0_0_240px] px-3 md:px-6 py-2 md:py-2 truncate">
                                        {user.email}
                                    </td>
                                    <td className="flex-[0_0_120px] md:flex-[0_0_140px] px-3 md:px-6 py-2 md:py-2 truncate">
                                        {user.role}
                                    </td>
                                    <td className="flex-[0_0_120px] md:flex-[0_0_140px] px-3 md:px-6 py-2 md:py-2">
                                        <span
                                            className={`px-2 md:px-3 py-1 rounded text-xs md:text-sm font-semibold ${getStatusStyle(
                                                user.status
                                            )} group-hover:ring-1 transition-all duration-300`}
                                        >
                                            {user.status}
                                        </span>
                                    </td>
                                            ...
                                            items={[
                                                    ...
                                                    : []),
                                                {
                                                    ...
                                                    onClick: (e) => {
                                                        e.stopPropagation();
                                                        navigate(
                                                            `/admin/user/activity-log/${user._id}`
                                                        );
                                                    },
                                                },
                                            ]}
                            ...
                            <tr className="flex w-full">
                                <td
                                    colSpan={7}
                                    className="flex-1 px-3 md:px-6 py-6 text-center text-gray-800 dark:text-blue-400"
                                >
                                    No users found.
                                </td>
                            </tr>
