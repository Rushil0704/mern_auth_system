...
export const AddEditUser = () => {
    ...
    const [emailExistsError, setEmailExistsError] = useState<string | null>(
        null
    );
    ...
    useEffect(() => {
        if (role === "admin" && status === "delete") {
            ...
            toast.info(
                "Admin users cannot be set to Deleted. Status reset to Active."
            );
        }
        ...
        const fetchUser = async () => {
            try {
                const res = await axios.get(
                    `${import.meta.env.VITE_BASE_ADMIN_URL}user/${userId}`,
                    {
                ...
                if (imgUrl) {
                    setPreviewUrl(
                        `https://s3.tebi.io/merndemo/users/${imgUrl}`
                    );
                }
            } catch {
    ...
    const checkEmailExists = async (email: string) => {
        ...
        try {
            const res = await axios.post(
                `${import.meta.env.VITE_BASE_URL}check-email`,
                { email }
            );
            setEmailExistsError(
                res.data.exists ? "Email already exists." : null
            );
        } catch {
    ...
    const onSubmit = async (data: AddEditUserType) => {
        ...
        try {
            ...
            formData.append("phone", data.phoneNo); // <-- Fix: use 'phone' instead of 'phoneNo'
            ...
            if (isEditMode) {
                await axios.put(
                    `${import.meta.env.VITE_BASE_ADMIN_URL}user/edit/${userId}`,
                    formData,
                    {
                        withCredentials: true,
                ...
                await axios.post(
                    `${import.meta.env.VITE_BASE_ADMIN_URL}user/create`,
                    formData,
                    {
                        withCredentials: true,
                        headers: { "Content-Type": "multipart/form-data" },
            ...
            <div className="w-full flex items-center justify-center my-auto h-full p-5 sm:p-10">
                <div className="w-full max-w-md mx-auto rounded-2xl shadow-2xl bg-white dark:bg-primaryBg p-4 sm:p-10 flex flex-col gap-4 sm:gap-8 border border-btnGrFromLight dark:border-darkBorder">
                    <div className="flex flex-col items-center gap-1 sm:gap-2">
                        ...
                        <h2 className="text-2xl font-bold dark:bg-gradient-to-r dark:from-textGrFrom dark:to-textGrTo bg-[linear-gradient(92.77deg,#080C1D_7.73%,#7E808C_99.72%)] bg-clip-text text-transparent">
                            Striked
                        </h2>
                    ...
                    <div className="flex flex-col items-center gap-2">
                        <label
                            ...
                            className="cursor-pointer bg-bgLight dark:bg-darkBorder w-fit rounded-full shadow-md"
                        >
                            {imageLoading ? (
                        ...
                        <div className="flex flex-col gap-3 sm:gap-4 w-full">
                            ...
                            <div className="flex gap-4">
                                <div className="flex flex-col flex-1">
                                    <label className="font-medium mb-1 text-gray-700 dark:text-btnGrToLight text-xs">
                                        ROLE{" "}
