    ...
    try {
        ...
        if (!existingUser && req.file) {
            ...
            const uploadCommand = new PutObjectCommand({
                Bucket: process.env.TEBI_BUCKET || "mydemo",
                Key: uniqueFileName,
...
export const getUserDetail = async (req: Request, res: Response) => {
    try {
        ...
        const user = await Users.findById({
            _id: req.userId,
            status: "active",
        }).select("firstName lastName email imgUrl location phoneNo");
        if (!user) {
...
export const userMessages = async (req: Request, res: Response) => {
    try {
        ...
        const users = await Users.aggregate([{ $match: { status: "active" } }]);

        const usersWithStatus = users.map((user) => ({
            ...user,
            isOnline: onlineUsers.has(user._id.toString()),
            lastSeen:
                lastSeenMap.get(user._id.toString())?.toISOString() || null,
        }));
