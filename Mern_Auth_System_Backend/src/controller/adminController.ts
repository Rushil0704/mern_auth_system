...
export const allUsers = async (req: Request, res: Response) => {
    try {
        ...
        const isStringField = ["firstName", "lastName", "email"].includes(
            sortField
        );
...
export const getAdminUserDetail = async (req: Request, res: Response) => {
    try {
        ...
        return res
            .status(200)
            .json({ user, message: "User Find successfully" });
    } catch (err) {
        return res
            .status(500)
...
export const editAdminController = async (req: Request, res: Response) => {
    try {
        ...
        const { firstName, lastName, email, role, status, location, phoneNo } =
            value;
        const userId = req.params.id;
